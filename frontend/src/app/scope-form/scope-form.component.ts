...
export class ScopeFormComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder, private scopeModelService: ScopeModelService, private router: Router, private route: ActivatedRoute) {
    this.scope = fb.group({
  ...
  create({ value, valid }: { value: Scope, valid: boolean }) {
    ...
    this.scopeModelService.create(applicationId, value)
      .subscribe((response: any) => {
        this.router.navigate(['dashboard/applications', applicationId, 'scopes']);
      });
  }
